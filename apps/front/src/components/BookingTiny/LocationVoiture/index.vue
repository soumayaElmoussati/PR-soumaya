  ...
  <div class="services">
    <div class="block">
      <div class="image">
        ...
        <h3>{{ $t("bookingTiny.servicesAnnexes.title")[7].title }}</h3>
        <p class="unity">
          {{ offer.price / 100
          }}{{ $t("bookingTiny.servicesAnnexes.title")[7].currency }}
        </p>
      </div>
      <div class="compteur">
        <div class="buttons">
        ...
        <div class="details">
          <textarea
            id="panier"
            v-model="questions"
            :placeholder="$t('bookingTiny.servicesAnnexes.title')[7].details"
            name="panier"
            rows="5"
            cols="33"
            type="textarea"
      ...
      <div class="price">
        <p>{{ convert() }}€</p>
        <p>{{ $t("bookingTiny.paragraphs.for") }} {{ numberLocation * offer.unity }} {{ $t("bookingTiny.paragraphs.car") }}</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    plus () {
      this.numberLocation++
      this.convert()
      this.$emit('totalLocation', this.totalLocation)
      this.$emit('numberLocation', this.numberLocation)
    },
    moins () {
      if (this.numberLocation === 0) {
    ...
    convert () {
      return (this.totalLocation =
        (this.offer.price / 100) * (this.numberLocation * this.offer.unity))
    },
    updateValue () {
...
.services {
  ...
  .block {
    ...
    .image {
      ...
      height: 155px;
      margin: 0 auto;
      background-image: url("../../../assets/img/servicesTiny/voiture.png");
