  ...
  <div class="services">
    <div class="block">
      <div class="image">
        ...
        <h3>{{ $t("bookingTiny.servicesAnnexes.title")[0].title }}</h3>
        <p class="unity">
          {{ offer.price / 100
          }}{{ $t("bookingTiny.servicesAnnexes.title")[0].currency }}
        </p>
      </div>
      <div class="compteur">
        <div class="buttons">
        ...
        <div class="details">
          <textarea
            id="panier"
            v-model="questions"
            :placeholder="$t('bookingTiny.servicesAnnexes.title')[0].details"
            name="panier"
            rows="5"
            cols="33"
            type="textarea"
      ...
      <div class="price">
        <p>{{ convert() }}€</p>
        <p>{{ $t("bookingTiny.paragraphs.for") }} {{ numberPanier * offer.unity }} {{ $t("bookingTiny.paragraphs.cart") }}</p>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    plus () {
      this.numberPanier++
      this.convert()
      this.$emit('totalPanier', this.totalPanier)
      this.$emit('numberPanier', this.numberPanier)
    },
    moins () {
      if (this.numberPanier === 0) {
        this.numberPanier = 0
        this.$emit('totalPanier', this.totalPanier)
        this.$emit('numberPanier', this.numberPanier)
      } else {
    ...
    convert () {
      return (this.totalPanier =
        (this.offer.price / 100) * (this.numberPanier * this.offer.unity))
    },
    updateValue () {
...
.services {
  ...
  .block {
    ...
    .image {
      ...
      height: 155px;
      margin: 0 auto;
      background-image: url("../../../assets/img/servicesTiny/panier.png");
