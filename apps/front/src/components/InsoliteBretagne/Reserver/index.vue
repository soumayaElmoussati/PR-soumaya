<template>
  <div class="contentReserver">
    <div class="title">
      <h3>{{ $t("insoliteBretagne.reserver.title") }}</h3>
      <hr>
      <h4> {{ $t("insoliteBretagne.titles.house") }} </h4>
    </div>
    <vueper-slides
      v-if="getHostOffers.length"
      :visible-slides="1"
      :gap="10"
      :slide-ratio="1 / 4"
    ...
    >
      ...
      <vueper-slide
        v-for="(offer, i) in getHostOffers"
        :key="i"
        :title="offer.title"
      >
    ...
    <div
      ...
      <img
        ...
        class="e_icon"
      >
      <h3 class="e-text body-1">
        {{ $t("insoliteBretagne.empty") }} <br >
      </h3>
    </div>
  </div>
</template>
...
export default {
  ...
  async fetch () {
    await this.$axios
      .get('/offers/bretagne')
      .then(res => {
        this.$store.dispatch('setHostOffers', res.data.all_offers)
      })
      .catch(err => {
