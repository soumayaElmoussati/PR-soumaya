...
import 'jquery'
import HomeHero from '@/components/Home/HomeHeroOffersTiny'
import OffersList from '@/components/OffersTinyList'
...
export default {
  ...
  head () {
    return {
      ...
      title: 'Séjour en Tiny-House, séjournez chez un agriculteur',
      titleTemplate: '%s - AgriVillage',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.$t('home.meta.tiny')
        }
      ]
    }
  },
  ...
  methods: {
    clickCallback: function (page) {
      ...
      var FinalQuery
      if (this.SearchQuery === null || this.SearchQuery === `/offers`) {
        FinalQuery = '/offers?page=' + page
      } else {
        FinalQuery = this.SearchQuery + '&page=' + page
      }
      if (this.getSearchQuery !== '' && this.getSearchQuery !== '/offers') {
        FinalQuery = this.getSearchQuery + '&page=' + page
      }
    ...
    loadMore () {
      ...
      this.$wait.start('loading more offers')
      this.$axios
        .get(`/offers?page=${meta.next_page || 0}`)
        .then(res => {
