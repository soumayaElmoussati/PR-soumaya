class AddScriptOfferPictureIssue < ActiveRecord::Migration[5.2]
  def up
    Offer.all.each do |offer|
      if offer.pictures.length > 0 && offer.cloud_pictures.length == 1
        ab = offer.cloud_pictures[0].public_id
        offer.pictures.each do |old_picture|
          if !old_picture.image.identifier.nil?
            if old_picture.image.identifier[13..-1] != ab
